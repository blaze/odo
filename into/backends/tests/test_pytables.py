...
class TestPyTablesLight(object):
    def test_read(self, table_file):

        with ensure_clean_store(path=table_file, datapath='/title') as t:
            shape = t.shape
            assert shape == (5,)

    def test_write_no_dshape(self, table_file):
        with pytest.raises(ValueError):
            PyTables(path=table_file, datapath='/write_this')

    def test_write_with_dshape(self, table_file):
    ...
    def test_table_into_ndarray(self, dt_table, dt_arr):
    ...
    def test_ndarray_into_table(self, dt_table, dt_arr):
    ...
    @pytest.mark.xfail(reason="Poor datetime support")
    def test_datetime_discovery(self, dt_table, dt_arr):
        with ensure_clean_store(dt_table, '/dt') as t:
            lhs, rhs = map(discover, (t, dt_arr))
            assert lhs == rhs

    def test_node_discover(self, dt_table):
