...
class TestAppendSimulation(unittest.TestCase):
    ...
    def test_csv_to_json_append(self):
        with filetext('alice,1\nbob,2', extension='.csv') as source:
            ...
            self.assertIsInstance(path.source, CSV)
            self.assertIsInstance(path.target, JSON)
            ...
            self.assertIn('def object_to_json', path.append_code)
    ...
    def test_list_to_set_append(self):
        ...
        self.assertIsInstance(path.source, list)
        self.assertIsInstance(path.target, set)
        ...
        self.assertIn('def object_to_set(a, b, **kwargs)', path.append_code)
...
class TestConvertSimulation(unittest.TestCase):
    ...
    def test_csv_to_dataframe_convert(self):
        with filetext('alice,1\nbob,2', extension='.csv') as source:
            ...
            self.assertIsNone(path.append_sig)
